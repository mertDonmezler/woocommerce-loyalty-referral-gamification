 ...
 * that belongs to the Gorilla Referral & Affiliate (RA) plugin.
 *
...
// ── Cross-Plugin Safety: detect if RA plugin is still active ──
// The gorilla_credit_log table and _gorilla_store_credit user meta are shared
// between LG (store credit) and RA (affiliate commission tracking).
// Only drop/delete them if RA is not active.
$active_plugins = get_option('active_plugins', array());
$ra_active = false;
foreach ($active_plugins as $p) {
    if (strpos($p, 'gorilla-referral-affiliate') !== false) {
        $ra_active = true;
        break;
    }
}
...
$options_to_delete = array(
    ...
    'gorilla_lr_smart_coupon_last_run',
    ...
    'gorilla_lr_meta_retention_days',
    'gorilla_lr_meta_cleanup_last_run',
);
...
// Store Credit user meta: only delete if RA is not active, since RA
// also adjusts store credit for affiliate commissions.
if (!$ra_active) {
...
// Social proof transient cleanup
delete_transient('gorilla_social_proof_events');

// Tier stats transient cleanup
delete_transient('gorilla_lr_tier_stats');

// Loyalty bar transients (per-user, clean by pattern)
$wpdb->query($wpdb->prepare(
    "DELETE FROM {$wpdb->options} WHERE option_name LIKE %s OR option_name LIKE %s",
    '_transient_gorilla_lr_bar_%',
    '_transient_timeout_gorilla_lr_bar_%'
));
